...
# Run 4: v12 SFT (loop-cleaned corpus) at 16384, packed, 100-step probe.
...
# A probe: 100 iterations x 768 packed sequences = 76,800 sequences consumed,
# well under one epoch of the packed cache (~400k sequences), so no wraparound
# and no epoch-length computation needed. If this is ever extended to a full
# epoch, compute the exact count offline like run 3 did.
export TRAIN_ITERS=100

export WANDB_ENTITY="adamo1139-no"
export WANDB_PROJECT="poziomka-sft"
export WANDB_NAME="poziomka_sft_run4_v12_16384_packed_100steps"
