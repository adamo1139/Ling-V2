...
gen_config = lambda args, tokenizer: f"""{{
    ...
    "moe_shared_expert_intermediate_size": {getattr(args, 'moe_shared_expert_intermediate_size', args.moe_ffn_hidden_size)}
}}
"""  # noqa: E731
...
class DCPKeyMapper:
    ...
    split_key_map = {
        # shared expert fc1 (gate+up) needs to be split
        "mlp.shared_experts.linear_fc1.weight": "mlp.shared_experts",
        # routed experts fc1 (gate+up)
...
class SafetensorPart:
    def __init__(self, idx, total, keys, nbytes, converter):
        ...
        self.idx = idx
        # Capture vocab sizing info from converter for child processes
        self.target_vocab_size = getattr(converter, 'target_vocab_size', None)
        self.padded_vocab_size = getattr(converter, 'padded_vocab_size', None)
    ...
    def rewrite(self):
        ...
        # Ensure class-level sizing info is visible in spawned workers
        DCPKeyMapper.target_vocab_size = self.target_vocab_size
        DCPKeyMapper.padded_vocab_size = self.padded_vocab_size
        ...
        # Trim padded vocab rows to match tokenizer vocab size
        tgt_vs = DCPKeyMapper.target_vocab_size
        if tgt_vs is not None:
            try:
                tgt_vs = int(tgt_vs)
                for key in ("model.word_embeddings.weight", "lm_head.weight", "model.embed_tokens.weight"):
                    if key in new_state_dict and isinstance(new_state_dict[key], torch.Tensor) and new_state_dict[key].dim() == 2:
                        rows = new_state_dict[key].size(0)
                        cols = new_state_dict[key].size(1)
                        if rows > tgt_vs:
                            logging.info(f"Trimming {key}: {rows}x{cols} -> {tgt_vs}x{cols} to match tokenizer vocab")
                            new_state_dict[key] = new_state_dict[key][:tgt_vs, :]
                        else:
                            logging.info(f"No trim needed for {key}: {rows}x{cols}")
            except Exception:
                pass
        # Extra robust pass: slice any 2D weight whose first dim equals padded_vocab_size
        pv = DCPKeyMapper.padded_vocab_size
        if pv and tgt_vs and pv > tgt_vs:
            for k, v in list(new_state_dict.items()):
                if isinstance(v, torch.Tensor) and v.dim() == 2 and v.size(0) == pv:
                    logging.info(f"Trimming by padded match {k}: {pv}x{v.size(1)} -> {tgt_vs}x{v.size(1)}")
                    new_state_dict[k] = v[:tgt_vs, :]
...
class DCPConverter:
    def __init__(self, ckpt, dst, force_bf16=False, force_fp8=False, mtp_as_extra_layer=False, override_tokenizer_path=None):
        ...
        # Infer padded and target vocab sizes for robust export
        try:
            md = self.reader.read_metadata().state_dict_metadata
            emb_meta = md.get('embedding.word_embeddings.weight')
            if emb_meta is not None and hasattr(emb_meta, 'size') and len(emb_meta.size) >= 2:
                DCPKeyMapper.padded_vocab_size = int(emb_meta.size[0])
                self.padded_vocab_size = DCPKeyMapper.padded_vocab_size
            else:
                self.padded_vocab_size = None
        except Exception:
            self.padded_vocab_size = None

        # Determine target vocab size from tokenizer (preferred) or args.vocab_size
        self.tokenizer = None
        target_vocab_size = None
        tok_path = getattr(self.args, 'spm_tokenizer_path', None)
        if override_tokenizer_path:
            tok_path = override_tokenizer_path
        if tok_path and os.path.exists(tok_path):
            try:
                ...
                target_vocab_size = int(getattr(self.tokenizer, 'vocab_size', None) or len(self.tokenizer))
            except Exception:
                target_vocab_size = None
        if target_vocab_size is None:
            try:
                target_vocab_size = int(getattr(self.args, 'vocab_size', 0) or 0) or None
            except Exception:
                target_vocab_size = None
        DCPKeyMapper.target_vocab_size = target_vocab_size
    ...
    def write_config(self):
        with open(os.path.join(self.dst, 'config.json'), 'w') as f:
            ...
            # Ensure HF config vocab_size matches tokenizer (avoid sampling into padded rows)
            try:
                if DCPKeyMapper.target_vocab_size:
                    config['vocab_size'] = int(DCPKeyMapper.target_vocab_size)
            except Exception:
                pass
            if self.args.fp8:
