...
class Partition(object):
    ...
    def is_parquet_file(self, file_path):
        """Check if file is a Parquet file by checking magic number"""
        try:
    ...
    def process_parquet_file(self, file_name):
        """Process Parquet file by streaming it and converting to JSONL format"""
        input_file_name, output_prefix = file_name
        print(f"Processing Parquet file: {input_file_name}")
        
        if not datasets_available:
            raise ImportError("datasets library is required to process Parquet files")
        
        # Load the Parquet file and filter to required columns
        ds = load_dataset('parquet', data_files=input_file_name, split='train')
        
        # Filter to only the required columns
        ds = ds.select_columns(self.args.json_keys)
        ...
        # Convert dataset to JSON lines and process
        encoded_docs = pool.imap(encoder.encode, 
                                (json.dumps(row) for row in ds), 
                                32)
    ...
    def process_json_file(self, file_name):
        ...
        # Check if input is a Parquet file
        if self.is_parquet_file(input_file_name):
            self.process_parquet_file(file_name)
            return
